...
def get_features():
    features = {
        ...
        "variances":{
            "name": "Variances",
            "class": FeatureVariances2D,
            "model": "2D",
            "data_type": Mode.IQ,
        },
        "averages_1d_IQ":{
            "name" : "Avaerages 1D IQ",
            "class" : Averages1D_IQ,
            "model" : "1D",
            "data_type" : Mode.IQ,
        },
...
class FeatureVariances2D:
    ...
    def extract_feature(self, win_data, win_params):
        ...
        data = {
            "avg_dist": np.full(win_len, np.var(peak[:, 0])),
            "avg_std": np.full(win_len, np.std(peak[:, 0])),
            "avg_ampl": np.full(win_len, np.var(peak[:, 1])),
            "avg_total": np.full(win_len, np.var(peak[:, 2])),
        }
...
class Averages1D_IQ:
    def __init__(self):
        ...
        self.data = {
            ...
            "avg_ampl":"Avg. ampl.",
            "avg_total": "Avg. signal",
            "abs_total": "Abs. signal",
            "glob_max": "Global. Maxima",
            "glob_min": "Global. Minima",
        }
        #text, value, limits
        self.options = [
            ("Start", 0.1, [0.06, 7], float),
            ("Stop", 0.3, [0.06, 7], float),
        ]
    def extract_feature(self, win_data, win_params):
        try:
            sensor_idx = win_params['sensor_idx']
            dist_vec = win_params['dist_vec']
            options = win_params['options']
            arr = win_data['iq_data'][sensor_idx, :, :]
        except Exception as e:
            print('iq_data not available!\n', e)
            return None
        ...
        data = {
            ...
            "avg_ampl": np.mean(peak[:, 1]),
            "avg_total": np.mean(peak[:, 2]),
            "abs_toal": np.abs(peak[:, 2]),
            "glob_max": np.max(peak[:, 2]),
            "glob_min": np.min(peak[:, 2]),
        }
        print(data)
        return data
    def get_options(self):
...
class IQ_rawdata:
    ...
    def extract_feature(self, win_data, win_params):
        ...
        if start >= stop:
            return None
        abs_data = np.zeros((win_len, data_len))
        phase_data = np.zeros((win_len, data_len))
        for i in range(win_len):
            abs_data[i] = abs(arr[:, i])
            for j in range(len(arr)):
                phase_data[i][j] = cmath.phase(arr[j, i])
            # peak[i, 2] = np.sum(arr[start:stop, i])
        data = {
            "amplitude" : np.array(abs_data),
